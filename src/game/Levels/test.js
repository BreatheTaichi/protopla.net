...
export default function earth(arena) {
    ...
    ship_canvas.width = 50;
    ship_canvas.height = 50;
    var ship_context = ship_canvas.getContext("2d");

    var alienShip = new Image();
    alienShip.onload = function () {
        arena.numberToLoad--;
        ship_context.drawImage(alienShip, 0, 0, 40, 40);
    };
    alienShip.src = alienShipPNG;

    var luna = new Image();
    luna.onload = function () {
        arena.numberToLoad--;
    };
    luna.src = lunaPNG;
    // luna_context.drawImage(luna, 0, 0, 1000, 1000);
    ...
    arena.loadingMessage =
        "Planet Earth and it's moon Luna, with the Dragon Nebula for a background";

    arena.ship.img = ship_canvas;
    arena.background = background;
    arena.images.push({ img: earth, xStart: 1, yStart: 1, xEnd: 45, yEnd: 45 });
    arena.images.push({
        img: luna,
        xStart: 53,
        yStart: 11,
    ...
    lunaBlocks.forEach(({ x, y }) => {
        arena.blocks.push({ x: x * arena.size, y: y * arena.size, type: "invisible" });
    });
    track.forEach(({ x, y }) => {
